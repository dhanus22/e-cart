...
const Register = () => {
  ...
  const [state, setState] = useState({
    username: "", email: "", gender: "", password: "", confirmPassword: ""
  })
  ...
  return (
    <section className='flex justify-center items-center min-h-[80vh] px-4'>
      <div className='flex flex-col justify-center items-center my-8 shadow-2xl rounded-2xl p-3 w-full max-w-sm'>
        <h1 className='text-2xl font-bold py-2 text-blue-500'>Registration Form</h1>

        <form onSubmit={handleSubmit}
          className='flex flex-col gap-3 w-full p-5 rounded-[15px] text-[13px] font-bold'>

          <div className='flex flex-col gap-0.5'>
            <label htmlFor="username">Username:</label>
            <input id="username" type="text" placeholder='username' name='username'
              value={state.username} onChange={handleChange} autoComplete="username"
          ...
          <div className='flex flex-col gap-0.5'>
            ...
            <input id="email" type="email" placeholder='email' name='email'
              value={state.email} onChange={handleChange} autoComplete="email"
              className='h-8 w-full shadow rounded-[3px] font-extralight px-1'/>
          </div>
              ...
              {["male", "female", "others"].map(g => (
                <div key={g} className='flex items-center gap-1'>
                  <input type="radio" id={g} name="gender" value={g}
                    checked={state.gender === g} onChange={handleChange}/>
          ...
          <div className='flex flex-col gap-0.5'>
            ...
            <div className='relative flex items-center'>
              <input id="password" type={showPassword ? "text" : "password"}
                name="password" placeholder='password' value={state.password}
